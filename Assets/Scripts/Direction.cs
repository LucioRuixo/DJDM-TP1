...
public class Direction : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if(Diferencia > DifMin && Diferencia < DifMax)
		{
			...
			{
			case Sentido.Der:
                    Camion.rotation *= Quaternion.AngleAxis(Diferencia * Sensibilidad * UnityEngine.Time.deltaTime, Camion.up);
                    Volante.localRotation *= Quaternion.AngleAxis(Diferencia * Sensibilidad * UnityEngine.Time.deltaTime, Vector3.up);
			...
			case Sentido.Izq:
                    Camion.rotation *= Quaternion.AngleAxis((-1) * Diferencia * Sensibilidad * UnityEngine.Time.deltaTime, Camion.up);
                    Volante.localRotation *= Quaternion.AngleAxis((-1) * Diferencia * Sensibilidad * UnityEngine.Time.deltaTime, Vector3.up);
				//Aux = Volante.localEulerAngles;
				//Aux.x -= Diferencia * Sensibilidad;
				//Volante.localEulerAngles = Aux;
				break;
